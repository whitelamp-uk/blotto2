...
CREATE PROCEDURE `cloneTablesFrom`(
  IN dbMake varchar(64) character set ascii
 ,IN dbResults varchar(64) character set ascii
)
BEGIN
  DECLARE tableTo varchar(64) character set ascii
  ;
  DECLARE tableFrom varchar(64) character set ascii
  ;
  PREPARE insrt FROM 'INSERT IGNORE INTO ? SELECT * FROM ?'
  ;
  SET @tableTo = '`blotto_change`'
  ;
  SET @tableFrom = CONCAT('`',dbMake,'`.`blotto_change`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_contact`', CONCAT('`',dbMake,'`.`blotto_contact`')
  ;
  SET @tableFrom = '`blotto_contact`', CONCAT('`',dbMake,'`.`blotto_contact`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_entry`', CONCAT('`',dbMake,'`.`blotto_entry`')
  ;
  SET @tableFrom = '`blotto_entry`', CONCAT('`',dbMake,'`.`blotto_entry`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_generation`', CONCAT('`',dbMake,'`.`blotto_generation`')
  ;
  SET @tableFrom = '`blotto_generation`', CONCAT('`',dbMake,'`.`blotto_generation`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_insurance`', CONCAT('`',dbMake,'`.`blotto_insurance`')
  ;
  SET @tableFrom = '`blotto_insurance`', CONCAT('`',dbMake,'`.`blotto_insurance`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_player`', CONCAT('`',dbMake,'`.`blotto_player`')
  ;
  SET @tableFrom = '`blotto_player`', CONCAT('`',dbMake,'`.`blotto_player`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_prize`', CONCAT('`',dbMake,'`.`blotto_prize`')
  ;
  SET @tableFrom = '`blotto_prize`', CONCAT('`',dbMake,'`.`blotto_prize`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_result`', CONCAT('`',dbResults,'`.`blotto_result`')
  ;
  SET @tableFrom = '`blotto_result`', CONCAT('`',dbResults,'`.`blotto_result`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
  SET @tableTo = '`blotto_supporter`', CONCAT('`',dbMake,'`.`blotto_supporter`')
  ;
  SET @tableFrom = '`blotto_supporter`', CONCAT('`',dbMake,'`.`blotto_supporter`')
  ;
  EXECUTE insrt USING @tableTo, @tableFrom
  ;
END$$
